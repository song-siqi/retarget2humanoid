...
BETAS_PATH = './betas_param/betas_r5_smpl_g1_neu2.npy'
HUMANOID_PATH = './humanoid_model/g1/g1_29dof_rev_1_0.xml'

MOTION_SOURCE = "./retarget_input/walking_backwards_smpl.npz"

def process_file(file_path, target_fps=1, smpl_model=None, betas_param=None, visualize=False):
    ...
    # list(data.keys())
    # >>> ['__key_strict__', '__data_len__', '__keypoints_compressed__', 'smpl', 'pred_cams',
    #      'bboxes_xyxy', 'image_path', 'person_id', 'frame_id']
    # data['smpl'].item().keys()
    ...
    betas_param = betas_param.reshape(1, 10).expand(num_frames, 10)
    body_model_output = smpl_model(
        betas=betas_param.to(torch.float32),
        body_pose=torch.from_numpy(body_pose_raw).to(torch.float32),
        global_orient=torch.from_numpy(global_orient).to(torch.float32),
        transl=torch.from_numpy(transl).to(torch.float32)
    )

    joints = body_model_output.joints.detach().cpu().numpy().reshape(num_frames, -1, 3)
    index_keypoints = [
    ...
    output['keypoint_trans'] = joints_keypoints[:, :, [2, 0, 1]]

    if visualize:
        visualize_3d(
            xyz=output['keypoint_trans'].reshape(-1, 3),
            file_name=file_name,
            key_name='keypoint',
            write_html=True
        )
